...
static void low_power_timer_wakeup_func(unsigned long data)
{
	...
	unsigned long int avg_sleep;
	unsigned long int avg_wake;
#ifdef TIMER_LOCK
	unsigned long flags;
	...
	if (__ratelimit(&ratelimit) && low_power_timer_trigger_cnt >= 2) {
		...
		DBG(0, "avg sleep(%lu/%d, %lu)us, avg wake(%lu/%d, %lu)us\n",
			low_power_timer_total_sleep,
			...
			avg_wake);
		DBG(0, "mode<%d,%d>, local balanced<%d,%d>\n",
...
void do_low_power_timer_test_work(struct work_struct *work)
{
	...
	while (!done) {
		...
		if (gdp_free_count == gdp_free_count_last) {
			signed int diff_time_ms;

			ktime_end = ktime_get();
			diff_time_ns =
				ktime_to_us(ktime_sub(ktime_end, ktime_begin));
			diff_time_ms = diff_time_ns/1000;
			set_time =
				low_power_timer_request_time - diff_time_ms;
...
int
mtk_kick_CmdQ(struct musb *musb, int isRx, struct musb_qh *qh, struct urb *urb)
{
	void __iomem        *mbase = musb->mregs;
	u16 intr_e = 0;
	...
	if (!mtk_is_qmu_enabled(hw_ep->epnum, isRx)) {
		...
		if (isRx) {
			...
			if (musb->is_multipoint) {
				DBG(4, "is_multipoint\n");
				musb_write_rxfunaddr(musb->mregs,
					hw_ep->epnum, qh->addr_reg);
				musb_write_rxhubaddr(musb->mregs,
					hw_ep->epnum, qh->h_addr_reg);
				musb_write_rxhubport(musb->mregs,
...
#endif
			/* turn off intrTx , will be revert by musb_ep_program*/
...
if (mtk_host_qmu_tx_max_number_of_pkts[hw_ep->epnum] < urb->number_of_packets)
	mtk_host_qmu_tx_max_number_of_pkts[hw_ep->epnum]
		...
		if (__ratelimit(&ratelimit)) {
			DBG(0, "TXQ[%d], max_isoc gpd:%d, max_pkts:%d\n",
			hw_ep->epnum,
			mtk_host_qmu_tx_max_active_isoc_gpd[hw_ep->epnum],
			mtk_host_qmu_tx_max_number_of_pkts[hw_ep->epnum]);

			DBG(0, "TXQ[%d], skip:%d, active_dev:%d\n",
			hw_ep->epnum,
			skip_cnt,
			mtk_host_active_dev_cnt);
			...
			DBG(1, "mode:%d, activate:%d, ep:%d-%s, act_cnt:%d\n",
					low_power_timer_mode,
			...
			if (low_power_timer_mode
				...
				if (urb->dev->speed == USB_SPEED_FULL) {
					unsigned i = gdp_used_count * 2 / 3;

					low_power_timer_sleep(i);
				} else {
					unsigned i = gdp_used_count * 2 / 24;
...
if (__ratelimit(&ratelimit)) {
	DBG(0, "RXQ[%d], max gpd:%d, max_pkts:%d, skip:%d, active_dev:%d\n",
		...
		if (urb->transfer_buffer_length < QMU_RX_SPLIT_THRE) {
			...
			urb->number_of_packets =
			((urb->transfer_buffer_length)
			+ QMU_RX_SPLIT_BLOCK_SIZE-1)/(QMU_RX_SPLIT_BLOCK_SIZE);
			if (gdp_free_count < urb->number_of_packets) {
				DBG(0, "free_count:%d, packets:%d\n",
					gdp_free_count, urb->number_of_packets);
				BUG();
