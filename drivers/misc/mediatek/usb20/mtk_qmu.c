...
int qmu_init_gpd_pool(struct device *dev)
{
	...
	for (i = isoc_ep_start_idx; i <= MAX_QMU_EP; i++) {
		if (isoc_ep_gpd_count > mtk_qmu_max_gpd_num) {
			Rx_gpd_max_count[i] = isoc_ep_gpd_count;
			Tx_gpd_max_count[i] = isoc_ep_gpd_count;
		} else {
			Rx_gpd_max_count[i] = mtk_qmu_max_gpd_num;
...
void mtk_qmu_insert_task(u8 ep_num, u8 isRx, u8 *buf, u32 length,
	u8 zlp, u8 isioc)
{
	QMU_INFO("ep_num:%d, isRx:%d, buf:%p, length:%d zlp:%d isioc:%d\n",
...
void qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	if (!req) {
		QMU_ERR("[RXD]Cant get next request of %d, but QMU has done.\n",
				ep_num);
		return;
	}
	request = &req->request;
	...
	QMU_INFO("[RXD]EP%d, Last=%p, Current=%p, End=%p\n",
	...
	QMU_INFO("[RXD] EP%d, Last=%p, End=%p, complete\n",
...
void qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	QMU_INFO("[TXD]EP%d, Last=%p, Current=%p, End=%p\n",
		 ep_num, gpd, gpd_current, Tx_gpd_end[ep_num]);
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		req = next_request(musb_ep);
		if (!req) {
			QMU_ERR("[TXD]Cant get request of %d, QMU has done.\n",
					ep_num);
			return;
		}
		request = &req->request;
	...
	QMU_INFO("[TXD]EP%d, Last=%p, End=%p, complete\n",
...
void h_qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	TGPD *gpd = Rx_gpd_last[ep_num];
	TGPD *gpd_current = (TGPD *)(unsigned long)
		MGC_ReadQMU32(base, MGC_O_QMU_RQCPR(ep_num));
	struct musb_hw_ep	*hw_ep = musb->endpoints + ep_num;
	...
	/*Transfer PHY addr got from QMU register to VIR addr*/
	gpd_current = (TGPD *)
		gpd_phys_to_virt((dma_addr_t)gpd_current, RXQ, ep_num);
	DBG(4, "\n");

	QMU_INFO("[RXD]EP%d, Last=%p, Current=%p, End=%p\n",
				ep_num, gpd, gpd_current, Rx_gpd_end[ep_num]);

	/* gpd_current should at least point
		to the next GPD to the previous last one */
	if (gpd == gpd_current) {
		QMU_ERR("[RXD][ERROR]gpd(%p) == gpd_current(%p)\n",
					gpd, gpd_current);

		QMU_ERR("[RXD][ERROR]EP%d RQCSR=%x, RQSAR=%x\n",
				ep_num,
				MGC_ReadQMU32(base, MGC_O_QMU_RQCSR(ep_num)),
				MGC_ReadQMU32(base, MGC_O_QMU_RQSAR(ep_num)));

		QMU_ERR("[RXD][ERROR]RQCPR=%x, RQLDPR=%x\n",
				MGC_ReadQMU32(base, MGC_O_QMU_RQCPR(ep_num)),
				MGC_ReadQMU32(base, MGC_O_QMU_RQLDPR(ep_num)));
		...
		QMU_ERR("[RX]HWO=%d, Next_GPD=%p ,EP=%d\n",
				(u32)TGPD_GET_FLAG(gpd),
				TGPD_GET_NEXT(gpd),
				(u32)TGPD_GET_EPaddr(gpd));
		QMU_ERR("[RX]BufLen=%d, Buf=%p,RLen=%d\n",
	...
	if (!gpd || !gpd_current) {
		QMU_ERR("[RXD][ERROR] EP%d, gpd=%p, gpd_current=%p, ishwo=%d\n",
				ep_num, gpd, gpd_current,
				((gpd == NULL) ? 999 : TGPD_IS_FLAGS_HWO(gpd)));
		QMU_ERR("[RXD][ERROR]rx_gpd_last=%p, RQCPR=0x%x\n",
				Rx_gpd_last[ep_num],
				MGC_ReadQMU32(base, MGC_O_QMU_RQCPR(ep_num)));
		return;
	}
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_ERR("[RXD][ERROR]EP%d RQCSR=%x,SAR=%x,CPR=%x,LDPR=%x\n",
				ep_num,
				MGC_ReadQMU32(base, MGC_O_QMU_RQCSR(ep_num)),
				MGC_ReadQMU32(base, MGC_O_QMU_RQSAR(ep_num)),
				MGC_ReadQMU32(base, MGC_O_QMU_RQCPR(ep_num)),
...
void h_qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	/* NORMAL EXEC FLOW */
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		QMU_INFO("[TXD]gpd=%p ->HWO=%d, BPD=%d, Next_GPD=%p\n",
			gpd, (u32)TGPD_GET_FLAG(gpd),
			(u32)TGPD_GET_FORMAT(gpd),
			TGPD_GET_NEXT(gpd));

		QMU_INFO("[TXD]gpd=%p DataBuffer=%p, BufferLen=%d\n",
			gpd,
			TGPD_GET_DATA(gpd),
			(u32)TGPD_GET_BUF_LEN(gpd));
...
void mtk_qmu_host_rx_err(struct musb *musb, u8 epnum)
{
	...
	if (rx_csr & MUSB_RXCSR_H_RXSTALL) {
		...
		DBG(0, "end %d HB incomplete ISO packet RX\n", epnum);
		status = -EPROTO;
	}
