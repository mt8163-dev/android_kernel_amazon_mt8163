...
#define ACCEL_MAX_OFFSET_VAL		786
#define ACCEL_MIN_OFFSET_VAL		-786
...
{
	if (accel_xyz_offset[0] > ACCEL_MAX_OFFSET_VAL ||
		accel_xyz_offset[0] < ACCEL_MIN_OFFSET_VAL) {
		return snprintf(buf, PAGE_SIZE,
		"offset_x=%d , out of range\nFail\n", accel_xyz_offset[0]);
	} else if (accel_xyz_offset[1] > ACCEL_MAX_OFFSET_VAL ||
				accel_xyz_offset[1] < ACCEL_MIN_OFFSET_VAL) {
		return snprintf(buf, PAGE_SIZE,
		"offset_y=%d , out of range\nFail\n", accel_xyz_offset[1]);
	} else if (accel_xyz_offset[2] > ACCEL_MAX_OFFSET_VAL ||
				accel_xyz_offset[2] < ACCEL_MIN_OFFSET_VAL) {
		return snprintf(buf, PAGE_SIZE,
		"offset_z=%d , out of range\nFail\n", accel_xyz_offset[2]);
	}
...
static void get_accel_idme_cali(void)
{
	...
	mm_segment_t fs;
	int offset[BMA250_DATA_BUF_NUM] = {0};

	cali_file = filp_open(BMA250_ACC_CALI_IDME, O_RDONLY, 0);
	if (IS_ERR(cali_file)) {
		GSE_LOG("read %s  ERROR\nFail\n", BMA250_ACC_CALI_IDME);
		return;
	}
	...
	cali_file->f_op->read(cali_file, (void *)offset,
		sizeof(offset), &cali_file->f_pos);
	set_fs(fs);
	filp_close(cali_file, NULL);
